...
int main(int argc,char *argv[]){
  ...
  if (my_rank == 0) {
    ...
    //(1)sprintf does not print, but stores text in c_string defined at the beginning
	...
	\tTask 0\n\
	\tRunning on %s\n", my_cpu_name);
    MPI_Isend (send_buffer, (unsigned)strlen(send_buffer) + 1, MPI_CHAR,
               1, 77, MPI_COMM_WORLD, &request);
    printf("hello there user, I've just started this send\n\
	and I'm having a good time relaxing.\n");
    /*mak sure that sending process waits and will not acces the send_buffer until all data is sent
    ...
    //(1)the from sprintf stored text is received
