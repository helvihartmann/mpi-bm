#include "mpi.h"
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
#include <errno.h>

//using namespace std;

#ifndef EXIT_SUCCESS
#define EXIT_SUCCESS 0
#define EXIT_FAILURE 1
#endif

int main( int argc, char *argv[] )
{
    int rank, size;
    int chunk = 128;
    int i;
    int *sb;
    int *rb;
    int status, gstatus;

    MPI_Init(&argc,&argv); //Adresse der Variabel und des Zeigers aufs erste Element des Arrays übergeben
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);//rank = number of process
    MPI_Comm_size(MPI_COMM_WORLD,&size);//size = number of all processes
    for ( i=1 ; i < argc ; ++i ) {
        if ( argv[i][0] != '-' )
	    printf("argv[%d][0]=%d\n",i,argv[i][0]);
            continue;
        switch(argv[i][1]) {
            case 'm'://wenn an der iten Stelle erste Zeile in argv m stand
                chunk = atoi(argv[++i]);//chunk enthält letzten parameter
	        printf("%d",argv[i][0]);	
                break;
            default:
                fprintf(stderr, "Unrecognized argument %s\n", argv[i]);fflush(stderr);
                MPI_Abort(MPI_COMM_WORLD,EXIT_FAILURE);
        }
    }//ende der for-Schleife
    sb = (int *)malloc(size*chunk*sizeof(int));
    if ( !sb ) {
        perror( "can't allocate send buffer" );fflush(stderr);
        MPI_Abort(MPI_COMM_WORLD,EXIT_FAILURE);
    }
    rb = (int *)malloc(size*chunk*sizeof(int));
    if ( !rb ) {
        perror( "can't allocate recv buffer");fflush(stderr);
        free(sb);
        MPI_Abort(MPI_COMM_WORLD, EXIT_FAILURE);
    }
    for ( i=0 ; i < size*chunk ; ++i ) {
        sb[i] = rank + 1;
        rb[i] = 0;
    }
   /* status = MPI_Alltoall(sb, chunk, MPI_INT, rb, chunk, MPI_INT, MPI_COMM_WORLD);
    MPI_Allreduce( &status, &gstatus, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD );
    if (rank == 0) {
        if (gstatus != 0) {
            printf("all_to_all returned %d\n",gstatus);fflush(stdout);
        }
    }*/
    free(sb);
    free(rb);
    MPI_Finalize();
    return(EXIT_SUCCESS);
}
