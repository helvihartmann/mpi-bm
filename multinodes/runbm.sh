...
#echo "#SBATCH --nodelist=lqcdn-r08n09" >> single.in
echo "" >> single.in
echo "" >> single.in
...
#echo "testib_parallel.sh "$n" "$n" | tee 55nodes_s"$s"_n"$END"_m"$m".log" >> single.in
    echo "mpirun --npernode 1 --mca btl_openib_if_include mlx4_0 build/multinodes -m 1 -i "$i" -w "$w" -e "$e" -o "$o" -q" $q "-s" $((n/2)) "-r" $((n/2)) "-b" $b ">> 55nodes_m1_s"$((n/2))"_n"$END"_"$node".out" >> single.in

    echo "mpirun --npernode 2 --mca btl_openib_if_include mlx4_0 build/multinodes -m 2 -i "$i" -w "$w" -e "$e" -o "$o" -q" $q "-s" $n "-r" $n "-b" $b ">> 55nodes_m2_s"$n"_n"$END"_"$node".out" >> single.in
    echo "" >> single.in
