...
do
    ...
    #sbatch pingpongSsend.in
        ...
        echo "" >> single.in
        echo "" >> single.in
        echo "mpirun --mca btl_openib_if_include mlx4_0 --mca btl_openib_eager_limit 128 build/pingpong -s "$m" -r "$m "-o 2 -e 10000000000 -p" $p "-x" $x >> single.in
        echo "" >> single.in
