...
do
    for w in 200 300 400 500 600 700 800 900 1000
    do
        touch single.in
        ...
        echo "#SBATCH --output="$x"gegen"$x"_s"$x"_q"$q"_w"$w"_i20g.out" >> single.in
        echo "#SBATCH --ntasks-per-node=2" >> single.in
        echo "#SBATCH --nodelist=node0" >> single.in
        echo "#SBATCH --distribution=cyclic" >> single.in
        echo "" >> single.in
        echo "" >> single.in
        echo "mpirun --mca btl_openib_if_include mlx4_0 build/multinodes -o 3 -i 20000 -b 8589934592 -m 2 -w "$w" -q" $q >> single.in
        echo "" >> single.in
