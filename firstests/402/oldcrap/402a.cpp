...
int main(int argc,char *argv[]){
    ...
    //int size, rank;
    // uint64_t nsize;
    size_t nsize, ncounts, size, rank;
    int ssize,rsize,i,j,m, k,l ;
    float z;
...
/* send the data*/
    ...
    printf("\n process %ld on %s checking for correct data transmisson \n",rank,name);
    m=-1;
    l=0;
    int everythingcorrect_check = 0;
    
	for(i=0;i<(nsize-ncounts);){//nsize-ncounts weil i in der k schleife noch um ncounts hochgezählt wird
        m++;
        for(k=0; k<ncounts; k++){
    ...
    if(everythingcorrect_check==1){
        printf("process %ld running found everything correct \n",rank);
    }
    else{
        printf("%d errors were spotted",l);
    }
    printf("Process %ld took %f seconds.\n",rank, (endtime-starttime));
